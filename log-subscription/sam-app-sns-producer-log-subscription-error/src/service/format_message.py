...
import base64
import gzip
from service.parser_service import StandardPathParser
...
def format_handler(event, requestId):
    ...
    try:
        ...
        if not compressed_payload:
            logger.error(
                "Erreur lors de la récupération des données de la subscription filter"
            )
        ...
        if not uncompressed_payload:
            logger.error(
                "Impossible de décomprésser le donnée depuis l'alert subscription filter"
            )
        ...
        message_content = f"Message: {log_data['logEvents'][0]['message']}"
        ...
        logger.error(
            f"Erreur lors de la formatage de message pour l'alerte subscription filter"
        )
